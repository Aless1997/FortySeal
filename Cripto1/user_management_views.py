...
def user_management_dashboard(request):
    ...
    # I superuser vedono tutti gli utenti
    if request.user.is_superuser:
        ...
        locked_users = UserProfile.objects.filter(organization=user_org, locked_until__gt=timezone.now()).count()
        recent_users = UserProfile.objects.filter(organization=user_org).select_related('user').order_by('-created_at')[:10]
...
def user_list(request):
    ...
    if request.user.is_superuser:
        ...
        user_org = request.user.userprofile.organization
        # Filtra gli utenti solo della stessa organizzazione
        users = UserProfile.objects.filter(organization=user_org).select_related('user', 'organization')
    ...
    context = {
        'page_obj': page_obj,
        'search': search,
        'username': username,  # Aggiungi al context
        'email': email,        # Aggiungi al context
        'status': status,
        'role_filter': role_filter,
        'all_roles': roles,    # Cambiato da 'roles' a 'all_roles'
    }
...
def edit_user(request, user_id):
    ...
    if request.method == 'POST':
        ...
        # Nella funzione edit_user, dopo il salvataggio del form
        # Nella funzione edit_user, dopo la riga 225 (if form.is_valid():)
        if form.is_valid():
...
def view_user_2fa_qrcode(request, user_id):
    ...
    # CORREZIONE: Usa qr.make_image() invece di qrcode.make_image()
